<script>
  ...
  import { notePc, spanColor, pcColor, chordRootPc, chordName, chordColorAt } from "../lib/sonofield.js";
  import { relativeMode } from "../lib/stores.js";
  ...
  const colorOf = (s) => spanColor(chords, s.start, s.end, tonicPc) || "#7a8499";
  ...
  const waveColorAt = (t) => mix(chordColorAt(chords, t, tonicPc) || "#56607a", "#39435a", 0.40);
  ...
  const chordColor = (lab) => {
    const r = chordRootPc(lab);
    return r == null ? "#5b6270" : pcColor(r, tonicPc);
  };
  ...
  function draw() {
    ...
    const peaks = song.peaks || [], mid = H / 2;
  ...
  onMount(() => {
    const ro = new ResizeObserver(() => { width = wrap.clientWidth; draw(); });
    ro.observe(wrap); width = wrap.clientWidth; draw();
      ...
      <div class="pop" bind:clientWidth={popW} style="left:{popLeft(s)}px; --c:{colorOf(s)}">
        ...
        {#if motif}
          <div class="pop-row"><span class="pop-cap">motif</span>
            <span class="chips">{#each motif.cycle as lab}<span class="chip" style="--c:{chordColor(lab)}">{chordName(lab, tonicPc, mode, $relativeMode) || lab}</span>{/each}{#if motif.reps > 1}<span class="reps">×{motif.reps}</span>{/if}</span></div>
        {/if}
        {#if showChords}
          <div class="pop-row"><span class="pop-cap">chords</span>
            <span class="chips">{#each ch as lab}<span class="chip" style="--c:{chordColor(lab)}">{chordName(lab, tonicPc, mode, $relativeMode) || lab}</span>{/each}</span></div>
        {/if}
        {#if ly.length}
